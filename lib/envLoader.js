 ...
 * Python is in.
 */
function detectRubyEnvLoader(installDir) {
  let declared = '';
  for (const file of ['Gemfile', 'gems.rb', 'Gemfile.lock']) {
    try { declared += fs.readFileSync(path.join(installDir, file), 'utf8'); } catch {}
  }
  for (const [gem, why] of [
    ['dotenv-rails', 'dotenv-rails is in the Gemfile'],
    ['dotenv', 'dotenv is in the Gemfile'],
    ['figaro', 'figaro is in the Gemfile'],
  ]) {
    if (new RegExp(`(^|[^\\w-])${gem}([^\\w-]|$)`, 'im').test(declared)) {
      return { mode: 'dotenv', evidence: why };
    }
  }
  try {
    if (fs.existsSync(path.join(installDir, 'config', 'credentials.yml.enc'))
      || fs.existsSync(path.join(installDir, 'config', 'application.rb'))) {
...
/**
 * Go's answer, which is usually "there isn't one".
 *
 * Go has no `.env` convention: a binary reads os.Getenv and the environment
 * comes from the process manager. godotenv exists but is opt-in, so the
 * honest default is `none` - which for Go still means an explicit
 * os.Getenv call, because that is how a Go program reads any variable.
 */
function detectGoEnvLoader(installDir) {
  try {
    const gomod = fs.readFileSync(path.join(installDir, 'go.mod'), 'utf8');
    if (/joho\/godotenv/.test(gomod)) return { mode: 'dotenv', evidence: 'godotenv is required in go.mod' };
  } catch {}
  return { mode: 'none', evidence: 'Go reads the process environment directly' };
}
...
export function detectEnvLoader(installDir, language = 'javascript') {
  ...
  if (normalizeLanguage(language) === 'ruby') return detectRubyEnvLoader(installDir);
  if (normalizeLanguage(language) === 'go') return detectGoEnvLoader(installDir);
