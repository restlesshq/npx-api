 ...
 * The plugin's support matrix: webpack builds on Next >= 13.4, Turbopack
 * builds on Next >= 15.3 (older Turbopack gets a build warning and NO
 * capture - a silent runtime failure, so the installer must fall back to
 * manual wrapping rather than ship it).
 *
 * When the version can't be resolved at all we assume a modern release:
 * an App Router layout implies >= 13.4 in practice, and the failure mode
 * of a wrong guess is a build-time warning, not a crash.
 *
 * Returns { supported, version, reason } - `reason` set when unsupported.
 */
export function nextAutoWrapSupport(rootDir) {
  const version = resolveNextVersion(rootDir);
  const mm = parseMajorMinor(version);
  if (!mm) {
    return { supported: true, version: null, reason: null };
  }
  const atLeast = (maj, min) => mm.major > maj || (mm.major === maj && mm.minor >= min);
  if (!atLeast(13, 4)) {
    return {
      supported: false,
      version,
  ...
  if (!atLeast(15, 3) && scriptsUseTurbopack(rootDir)) {
    return {
      supported: false,
      version,
