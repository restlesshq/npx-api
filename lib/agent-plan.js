...
function facts({ mapped, signals }) {
  ...
  if (mapped.length) {
    ...
    lines.push('Frameworks found by the deterministic scan (no model involved, so verify before trusting):');
    for (const s of signals.slice(0, 6)) {
      lines.push(`- \`${s.file || s.rootDir || '?'}\`${s.framework ? ` - ${s.framework}` : ''}`);
    }
  } else {
    lines.push('Nothing is mapped yet, and the file scan did not spot a framework it recognizes. Find the API yourself before starting.');
  }
  return lines.join('\n');
}
...
1. Get the project's key. This one is ours: it generates the key, registers the
   ...
   It prints \`{ "apiKey": "...", "projectId": "...", "envFile": "..." }\`. Do not invent a
   key, and do not paste the returned key into source - it belongs in \`.env\`.
...
Ask the user to start their server, then point the check at the base URL you
established in step 1 (host, port, and any mount prefix - don't assume \`:3000\`,
and if the app is containerized use the published port, not the internal one):
