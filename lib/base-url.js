...
export function isPlausibleBaseUrl(url) {
  ...
  // Private / loopback / link-local IPv4 ranges are dev addresses.
  if (/^(10\.|127\.|192\.168\.|169\.254\.|172\.(1[6-9]|2\d|3[01])\.)\d/.test(host)) return false;
  return true;
}
...
/**
 * Gate for a spec that is about to be recorded and eventually uploaded:
 * is `servers[0].url` something we're willing to publish as this API's
 * base URL? Three outcomes:
 *
 *   ok + url       - a plausible public URL (normalized).
 *   ok + relative  - a path-only server ("/", "/v1"): the honest form for
 *                    an API with no public URL. Nothing to publish.
 *   not ok         - localhost, a loopback/container host, a dev port -
 *                    an address that must not ship to the dashboard.
 *
 * A missing/empty servers array is fine (OAS defaults it to "/").
 */
export function checkOasServers(oas) {
  const first = oas?.servers?.[0]?.url;
  if (!first || typeof first !== 'string' || !first.trim()) return { ok: true, url: null };
  const url = first.trim();
  if (url.startsWith('/')) return { ok: true, url, relative: true };
  if (isPlausibleBaseUrl(url)) return { ok: true, url: normalize(url) };
  return { ok: false, url };
}
