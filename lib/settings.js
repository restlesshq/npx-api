...
/**
 * Normalize an `apis[].rootDir` before comparing it. `api`, `api/` and `./api`
 * all name one directory, and once they stopped matching, a `--dir` typed with
 * a trailing slash created a SECOND entry for an API that already had one.
 * Two entries make `resolveApi` throw out of `restless()`, so the settings
 * file the CLI wrote would fail the user's server at boot.
 */
export function apiDirKey(dir) {
  const norm = path.normalize(String(dir || '.')).replace(/\\/g, '/').replace(/\/+$/, '');
  return norm === '' || norm === '.' ? '.' : norm;
}

/**
 * Re-find an API entry inside a freshly-loaded settings file.
 *
 * Every writer has to do this - load, find, mutate, save - and they were doing
 * it three times with two different rules, so a workspace where `id` and
 * `projectId` disagreed could have one writer update an entry and another
 * silently update nothing. `id` first because it is the stable local handle;
 * `projectId` second because an entry adopted by a different checkout may not
 * have an `id` yet.
 */
export function findApiEntry(settings, apiEntry) {
  const apis = settings?.apis || [];
  return (
    (apiEntry?.id && apis.find((a) => a.id === apiEntry.id))
    || (apiEntry?.projectId && apis.find((a) => a.projectId === apiEntry.projectId))
    || null
  );
}
...
/**
 * The fields `npx restless update` may change, and how to validate each one.
 *
...
/**
 * The scratch paths we write INSIDE the committed `.restless/` directory, kept
 * out of git from the directory itself.
 *
 * `update` stages a refreshed spec into `.oas-refresh/` so nothing reaches the
 * developer's spec before they agree to it, and deletes it again on the way
 * out - via an `exit` handler, a SIGINT handler, and a sweep at the start of
 * the next run. None of those survive a SIGKILL or a crashed process, and the
 * directory they'd leave behind sits in a directory people are told to commit.
 * A `.gitignore` covers the case the handlers can't.
 */
const DIR_GITIGNORE = `# Scratch files npx restless writes here. Everything else in
# this directory is source - see README.md - and belongs in your commits.
.oas-refresh/
.oas-candidates.json
`;
...
export function saveSettings(cwd, settings) {
  ...
  // Written when absent, never rewritten - if someone edits or deletes either
  // of these, that's their call to keep. Absent rather than on-creation so an
  // install that predates them picks them up on its next save.
  for (const [name, body] of [['README.md', DIR_README], ['.gitignore', DIR_GITIGNORE]]) {
    const file = path.join(dir, name);
    if (!fs.existsSync(file)) {
      try { safeWriteFileSync(file, body); } catch {}
    }
  }
