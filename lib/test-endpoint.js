 ...
 *
 * We deliberately send the request WITHOUT the target API's auth. The test
 * step doesn't need a successful call - it only needs to see the request
 * flow through the Restless SDK (which stamps every response, including a
 * 401, with its diagnostic header). Omitting auth means the user never has
 * to paste a real API key: an unauthenticated request that comes back as a
 * 401 proves the wiring just as well as a 200 would.
 */
