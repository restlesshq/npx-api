 ...
 * not broken. Returns { oas, settings, endpoints, error? } where oas and
 * settings are 'uploaded' | 'none' | 'failed'.
 */
export async function uploadPendingArtifacts({ rootDir, projectId, setupKey, fetchImpl = fetch }) {
  const settings = loadSettings(rootDir);
  const entry = (settings.apis || []).find((a) => a.projectId === projectId) || settings.apis?.[0];
  ...
  if (oasPath && fs.existsSync(oasPath)) {
    try {
      ...
      if (res.ok) {
        result.oas = 'uploaded';
        if (isJson) { try { result.endpoints = countOperations(JSON.parse(oasRaw)); } catch {} }
      } else {
        result.oas = 'failed';
  ...
  if (Array.isArray(settings.apis) && settings.apis.length) {
    try {
      ...
      result.settings = res.ok ? 'uploaded' : 'failed';
    } catch {
