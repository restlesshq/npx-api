...
export function fixContext(state, { localBase, cli = 'api' } = {}) {
  if (state === 'stale-key') {
    return {
      ...
      guidance:
        `This is not a code problem, so don't edit the wiring. Run \`npx ${cli} key\` to reconcile the ` +
        `key on disk with the registered project, restart the server so it picks up any change to .env, ` +
        `and re-run the check. Never read .env yourself and never invent a key.`,
    };
  }
  if (state === 'no-key') {
    return {
