import { bold, dim, green, red, yellow, cyan, dimSpinner } from './ui.js';
...
export function describeDiagnosis(state, { status, localBase = 'your server', aiTool = 'the AI', attempt = 0, frame = 0 } = {}) {
  switch (state) {
    ...
    case 'unreachable': {
      // The user has to go start their dev server themselves - so this must
      // read as "your turn", not "we're working on it". Lead with an amber
      // action imperative; demote the poll to a small dim "checking…" sub-line
      // (a spinner as the primary glyph would imply we're doing the work). The
      ...
      // After a handful of silent probes (~10-15s at the 2s poll interval) with
      // nothing answering, escalate the copy from "checking…" to a nudge.
      const checkLine = attempt >= 6
        ? yellow(`Still nothing on ${port || host} - have you started your server yet?`)
        : `${dimSpinner(frame)} ${dim(`checking ${host}…`)}`;
      return {
        icon: yellow('⚠'),
        canFix: false,
        lines: [
