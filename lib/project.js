...
export function findGitRoot(from) {
  const endSpan = timings.start('git rev-parse', { kind: timings.KINDS.EXEC });
  try {
...
export function isGitIgnored(file, cwd) {
  // Timed under one shared label rather than per file: what matters is
  // "git check-ignore x 40 = 2s", and a label per path would bury that in
  // forty rows of the operations table.
  const endSpan = timings.start('git check-ignore', { kind: timings.KINDS.EXEC });
  try {
