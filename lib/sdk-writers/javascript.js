 ...
 * code on re-runs - we can only patch known lines (init arg, project.id)
...
/**
 * Update (or insert) the `project: { id: <expr> },` field inside the
 ...
 *
 * Two modes:
 *   1. project.id is already present - swap just the expression so we
 *      keep the line shape and trailing comma.
 *   2. project.id is missing - insert a new line right after `apiKey:`,
 *      matching whatever indent the apiKey line uses (so the file ends
 *      up looking the same whether the AI or the user originally wrote
 *      the block).
 *
 ...
 */
export function setProjectId(content, expr) {
  ...
  const existingRe = /(project\s*:\s*\{[^{}]*?\bid\s*:\s*)([^,\n}]+)([},])/;
  if (existingRe.test(block)) {
    block = block.replace(existingRe, (_m, prefix, value, closer) => {
      // Preserve whatever whitespace the original had between the
      // expression and the closer, so we render `{ id: x }` (with the
      // trailing space) rather than `{ id: x}`.
    ...
    const projectLine = `${indent}project: { id: ${newExpr} },`;
 ...
 * inside the setup callback - the credential extraction and project.id
