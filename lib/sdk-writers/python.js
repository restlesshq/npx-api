...
export function readBlockFields(blockText) {
  ...
  // mistaken for it.
  const ownerAnchor = blockText.match(new RegExp(ownerAnchorFor(F.owner)));
  if (ownerAnchor) {
...
export function setOwnerId(content, expr) {
  ...
  const ownerRe = new RegExp(
    `(${ownerAnchorFor(F.owner)}\\{[^{}]*?${key(F.ownerId)}\\s*:\\s*)([^,\\n}]+)([},])`,
  );
  ...
  } else if (new RegExp(ownerAnchorFor(F.owner)).test(block)) {
