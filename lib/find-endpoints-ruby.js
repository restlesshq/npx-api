...
const IGNORE_DIRS = new Set([
  '.git', '.restless', 'node_modules', 'vendor', 'tmp', 'log', 'coverage',
  'public', '.bundle', 'bin', 'db',
]);
...
function walk(dir, sourceFiles, manifests, depth) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (MANIFESTS.includes(entry.name) || entry.name.endsWith('.gemspec')) manifests.push(full);
      else if (SOURCE_EXTS.has(path.extname(entry.name)) || entry.name === 'config.ru') {
