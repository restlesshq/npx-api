 ...
 *
 * `go list -m` answers from the module graph, so a `replace` directive
 * pointing at a sibling checkout - the Go equivalent of an editable install,
 * and how the test-apis fixtures consume the SDK before it is published -
 * counts exactly like a published requirement. Reading go.mod by hand would
 * see the `require` line but miss whether it actually resolves.
 *
 * Falls back to a go.mod grep when the toolchain is unavailable, so a machine
 * without Go installed reports "not installed" rather than crashing.
 */
function resolveInstalledGoSdk(packageDir) {
  const module = 'github.com/restlesshq/go';
  try {
    const out = execFileSync('go', ['list', '-m', module], {
      cwd: packageDir, encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'], timeout: 30000,
    }).trim();
    if (out && !/^go: /.test(out)) {
      debug.log('install-target.go-sdk', { out });
      return out;
    }
  } catch {
    // No toolchain, not a module, or the requirement is absent.
  }
  try {
    const gomod = fs.readFileSync(path.join(packageDir, 'go.mod'), 'utf8');
    if (new RegExp(`^\\s*(?:require\\s+)?${module.replace(/\./g, '\\.')}\\s+v`, 'm').test(gomod)) {
...
export function resolveInstalledSdk(packageDir, language) {
  ...
  if (canonical === 'go') return resolveInstalledGoSdk(packageDir);
...
export function describeMissingSdk(packageDir, language) {
  ...
  if (canonical === 'go') {
    return [
      `\`go list -m ${packageSpecifier}\` in ${packageDir} does not resolve it.`,
