...
export function suppressInput() {
  ...
  const onData = (key) => {
    if (isAbortKey(key)) debug.flushAndExit(0); // Ctrl-C or Esc still aborts.
  };
...
const LOGO_SUFFIXES = (subtitle) => ['', `    ${bold('Restless')}`, `    ${dim(subtitle)}`, ''];
...
/**
 * Run `fn` with `console.log` swallowed.
 *
 * Two reasons, both real:
 *
 *  - Some helpers narrate their own progress ("Spec ready at ...", "Downloaded
 *    to ..."), which is right when the path they name is the destination and
 *    wrong when it's a scratch file the caller is about to move. The caller
 *    knows which; the helper doesn't.
 *  - Anything written while a spinner is running interleaves with its redraw
 *    and corrupts the line, because the spinner owns that row.
 *
 * `process.stdout.write` is deliberately left alone, so a spinner keeps
 * animating while the chatter behind it is hidden.
 */
export async function withSuppressedOutput(suppress, fn) {
...
export function waitForKey() {
  ...
  return new Promise((resolve) => {
    ...
    stdin.once('data', (key) => {
      try { stdin.setRawMode(false); } catch {}
      stdin.pause();
      if (isAbortKey(key)) debug.flushAndExit(0);
...
export function waitForServerOrKey(probe, { intervalMs = 2000, spinnerMs = 180, render } = {}) {
  return new Promise((resolve) => {
    ...
    function onKey(key) {
      if (stopped) return;
      if (isAbortKey(key)) { cleanup(); debug.flushAndExit(0); return; }
