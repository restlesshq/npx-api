 ...
 * `render(attempt)` is called before each probe so the caller can redraw
 * a "still waiting…" line (with a spinner frame or the attempt count).
 *
 * Resolves with either:
 *   { type: 'reachable', result }   - probe returned a non-unreachable state
 *   { type: 'key', key }            - the user pressed a key
 */
export function waitForServerOrKey(probe, { intervalMs = 2000, render } = {}) {
  return new Promise((resolve) => {
    const { stdin } = process;
    let stopped = false;
    let timer = null;
    ...
    async function tick() {
      if (stopped) return;
      attempt++;
      if (render) { try { render(attempt); } catch {} }
      let result;
