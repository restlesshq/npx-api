...
export async function terminalRunScreen(defaultCommand, { onRun, pollConfig, noLogsHint, noLogsHintMs = 8000 } = {}) {
  ...
  function buildScreen(text, cp, output, phase) {
    ...
    if (phase === 'edit') {
      ...
      lines.push(`  ${bc}↵ Enter${rst}${d} run again · ${rst}${bc}e${rst}${d} edit · ${rst}${bc}Tab${rst}${d} continue setup${rst}`);
    } else {
    ...
    const action = await new Promise((resolve) => {
      ...
      const handler = (key) => {
        if (key === '\r' || key === '\n') {
          ...
          stdin.removeAllListeners('data');
          if (result.success && pollConfig) {
            // In polling mode, Enter reruns the same command so the user
            // can keep firing requests and watch logs accumulate above.
            resolve('rerun');
