 ...
 * Two modes:
 *
 * 1. No `loadingText` — simple spinner → colored ● + status code.
 *    await inlineStatus({ code: '400 Bad Request', success: false });
 *
 * 2. With `loadingText` — spinner + loading label animates during the wait,
 *    then BOTH get erased and replaced with the colored ● + status code.
 *    Useful for showing a "request in flight" that transforms into its result.
 *    await inlineStatus({ code: '400 Bad Request', success: false, loadingText: 'GET /api/endpoint' });
 *
 * Valid styles: arc, halfcircle, piefill, pulse, sparkle, concentric, braille.
 */
export async function inlineStatus({ code, success, duration = 1100, style = 'arc', loadingText = '' }) {
  ...
  if (loadingText) {
    // Transform mode: spinner + loadingText → resolved state (erases both).
    process.stdout.write(dim(frames[0]) + ' ' + dim(loadingText));

    // Animate the spinner in place using ANSI save/restore cursor, leaving
    // the loading text untouched.
    const stepsBack = 1 + loadingText.length + 1; // text + space + spinner
    const interval = setInterval(() => {
      frame = (frame + 1) % frames.length;
      process.stdout.write('\x1b[s');                          // save cursor
      process.stdout.write(`\x1b[${stepsBack}D`);              // back to spinner
      process.stdout.write(dim(frames[frame]));
    ...
    // Erase spinner + space + loadingText, briefly pause, then write the
    // resolved status in place. Reads as "the in-flight request landed".
    const totalLen = 1 + 1 + loadingText.length;
    process.stdout.write(`\x1b[${totalLen}D\x1b[K`);
    await new Promise((r) => setTimeout(r, 140));
    process.stdout.write(`${circle} ${colored}`);
    return;
  }

  // Simple mode: spinner alone → colored ● + status code text.
...
export async function terminalRunScreen(defaultCommand, { onRun, pollConfig } = {}) {
  ...
  async function pollLogs() {
    ...
    try {
      ...
      if (!res.ok) {
        process.stderr.write(`[poll] HTTP ${res.status} from ${pollConfig.url}\n`);
      }
      if (res.ok) {
        const data = await res.json();
        process.stderr.write(`[poll] got ${data.logs?.length ?? 0} logs\n`);
      ...
      // Debug: log poll errors to stderr so they don't interfere with the TUI
      process.stderr.write(`[poll error] ${err.message || err}\n`);
    }
  }
  ...
  function buildTopHalf() {
    ...
    if (logs.length === 0) {
      ...
      for (const log of visible) {
        ...
        const duration = log.duration != null ? `${d}${Math.round(log.duration)}ms${rst}` : '';
