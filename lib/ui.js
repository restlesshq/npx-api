...
const LOGO_LINES = [
  _logoMk([['─', dim], [' ', _logoId], ['▬▬▬▬▬▬▬', green], [' ', _logoId], ['─────', dim]]),
  _logoMk([['────', dim], [' ', _logoId], ['▬▬▬▬▬▬', green], [' ', _logoId], ['───', dim]]),
  _logoMk([['──', dim], [' ', _logoId], ['▬▬▬▬▬▬', green], [' ', _logoId], ['─────', dim]]),
  _logoMk([['─', dim], [' ', _logoId], ['▬▬▬', green], [' ', _logoId], ['──', dim], [' ', _logoId], ['▬▬▬', green], [' ', _logoId], ['──', dim]]),
];
const LOGO_SUFFIXES = ['', `    ${bold('Restless')}`, `    ${dim('npx api setup')}`, ''];
...
export async function terminalRunScreen(defaultCommand, { onRun, pollConfig, noLogsHint, noLogsHintMs = 8000 } = {}) {
    ...
    if (logs.length === 0 && !pendingEntry) {
      ...
      if (pendingEntry) {
        if (pendingEntry.state === 'pending') {
          ...
          renderFailedRow(lines, pendingEntry);
        }
      }

      // Past failed attempts so reruns leave a visible trail.
      for (const att of failedAttempts) {
        renderFailedRow(lines, att);
      }
  ...
  function renderFailedRow(lines, entry) {
    lines.push(formatLogRow({
      icon: `${yc}⚠${rst}`,
      status: `${d}---${rst}`,
      method: entry.method,
