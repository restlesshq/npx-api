...
export async function terminalPreview(command, { cta = 'Press ENTER to run it', note = '' } = {}) {
  ...
  await new Promise((resolve) => {
    const onKey = (key) => {
      if (key === '\x03') { stdin.setRawMode(false); stdout.write('\x1b[?25h\n'); debug.flushAndExit(0); return; }
      // Any key continues, same as the keypress gate this replaces - but
      // ENTER is what the footer promises, so it's what most people press.
      stdin.removeListener('data', onKey);
      stdin.setRawMode(false);
      stdin.pause();
      stdout.write('\x1b[?25h\n\n');
      debug.log('input.terminalPreview', { command, key: key === '\r' ? 'enter' : 'other' });
      resolve();
    };
    stdin.on('data', onKey);
  });
