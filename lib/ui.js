 ...
 * run history (200 received? 3+ failed? still warming up?) and returns
 * `{ kind, lines }` where `kind` is one of `'success' | 'failing' |
 * 'placeholder' | 'default'` and `lines` is the rendered ANSI strings.
 *
...
export function pickContextualHint({
  ...
  const had200 = Array.isArray(logs) && logs.some((l) => l && l.status >= 200 && l.status < 300);
  const failedCount =
  ...
  if (failedCount >= 3) {
    return {
      kind: 'failing',
      lines: [
        `  ${yellow('Edit the code snippet below to make a valid call.')}`,
        `  ${dim("If you'd rather skip ahead and fix this later, press ")}${bold('Tab')}${dim(' to continue.')}`,
      ],
    };
  }
  const hasPlaceholder = typeof command === 'string' && command.includes('API_KEY_HERE');
  return {
    kind: hasPlaceholder ? 'placeholder' : 'default',
    lines: [
      `  Make a successful call to confirm everything's set up.`,
      hasPlaceholder
        ? `  ${dim('Edit the snippet below - replace ')}${bold('API_KEY_HERE')}${dim(' with a real API key for testing.')}`
        : `  ${dim('Edit the snippet below to make a valid request.')}`,
...
export function actionPicker(items, { message, actions = [], defaultIndex = 0 } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    function buildLines() {
      const all = [''];
      all.push(`  ${bold(message)}`);
      all.push('');
      for (let i = 0; i < items.length; i++) {
        for (const r of itemRows(items[i], i)) all.push(r);
      }
      // Each action gets two blank lines of separation so Submit reads
      // as a distinct call-to-action and any `afterthought` action
      // (Chat) reads as visibly secondary.
...
export async function terminalRunScreen(defaultCommand, { onRun, pollConfig, noLogsHint, noLogsHintMs = 8000 } = {}) {
  // Non-interactive: no full-screen TUI and no waiting for the user to hit
  // Enter. Run the command once against the default, print what came back,
  // and return. `onRun` still executes (it's where the caller reads the
  // response headers), so the diagnostic the caller relies on is populated.
  if (!isInteractive()) {
    console.log(`  ${dim('$')} ${defaultCommand}`);
    let result = { output: '', success: true, command: defaultCommand };
    if (onRun) {
      try {
        result = onRun(defaultCommand) || result;
      } catch (err) {
        result = { output: err?.message || String(err), success: false, command: defaultCommand };
      }
    }
    const command = typeof result.command === 'string' ? result.command : defaultCommand;
    if (result.output) console.log(String(result.output).split('\n').map((l) => `  ${l}`).join('\n'));
    debug.log('input.terminalRunScreen', { command, success: result.success, auto: true });
    return { command, output: result.output || '', success: result.success !== false };
  }
  const { stdin, stdout } = process;
  const termWidth = stdout.columns || 80;
  ...
  while (true) {
        ...
        if (key === '\t') {
          if (pollConfig && !hasSuccessfulRun) {
            ...
            drawScreen(buffer, cursor, [
              '',
              `  ${yc}Are you sure you want to proceed before making a valid call?${rst}`,
    ...
    if (editAction.action === 'continue') {
      cleanup();
      return { command, ...(lastOutput ? { output: lastOutput.join('\n'), success: true } : { output: '', success: true }) };
    }
