...
- **Write the JSON COMPACT - no indentation, no newlines between keys, no spaces after `:` or `,`.** One long line is exactly what we want. The CLI re-indents the file for you the moment you finish, so the committed file ends up properly formatted either way; every space you emit yourself is a token the user waits for. On a real spec this is about a third of the output.
...
- **The source files above are already in this prompt - do not re-read them.** Only reach for the Read tool for a file that is genuinely missing from it and that you actually need. Every tool call costs the user several seconds of waiting.
