...
- Register the middleware/plugin **BEFORE route definitions** so it captures all requests. (Next.js is the exception - there is no middleware registration; use `withRestless` + `restless.config` on the App Router, wrap route handlers with `@restlessai/sdk/next` on the Pages Router, and never touch `middleware.ts` / `proxy.ts`.)
